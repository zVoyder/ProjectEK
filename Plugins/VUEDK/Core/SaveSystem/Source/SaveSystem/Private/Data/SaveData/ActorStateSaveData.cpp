...
bool UActorStateSaveData::SaveObjectDataNative(UObject* ObjectToSave)
{
	if (!Check())
		return false;
	...
	if (!IsValid(Actor))
	{
		UE_LOG(LogSaveSystem, Warning, TEXT("UActorStateSaveData::SaveObjectDataNative: ObjectToSave is not a valid Actor."));
		return false;
	}
